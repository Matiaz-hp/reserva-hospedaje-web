...
async function cargarHoteles(filtro=''){
  ...
  snapshot.forEach(docSnap=>{
    ...
    card.innerHTML = `<img src="assets/images/${h.imagen}" alt="${h.nombre}"><h3>${h.nombre}</h3><p>Ciudad: ${h.ciudad}</p><p>Precio: $${h.precio}</p>`;
...
async function cargarHotelesMapa(){
  ...
  snapshot.forEach(docSnap=>{
    const h = docSnap.data();
    if(h.lat && h.lng){ L.marker([h.lat,h.lng]).addTo(map).bindPopup(`<b>${h.nombre}</b><br>${h.ciudad}`); }
  });
}

window.addEventListener('DOMContentLoaded',()=>{ cargarHoteles(); cargarHotelesMapa(); });
